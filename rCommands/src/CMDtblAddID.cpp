...
int main(int argc, char *argv[]) {
    ...
    char *idFieldName = (char *) NULL;
    DBObjTable *table;
    DBObjTableField *idField;
    DBObjRecord *record;
    ...
    idFieldName = idFieldName == (char *) NULL ? (char *) "IDfield" : idFieldName;
    idField = table->Field (idFieldName);
    if (idField == (DBObjTableField *) NULL) {
        idField  = new DBObjTableField (idFieldName, DBTableFieldInt, (char *) "%10d", sizeof (DBInt));
        table->AddField(idField);
    }
    for (recID = 0; recID < table->ItemNum(); ++recID) {
        record = table->Item(recID);
        idField->Int(record,recID + 1);
    }
